...
$products = [];
$products = $produitC->afficherProduit(); 
?>
            ...
            <div class="col-12">
                ...
                <form class="row g-3" id="form" name="form" method="POST" action="">
                    ...
                    <div class="col-sm-6 col-md-3">
                        ...
                        <input type="text" class="form-control" id="adresse" name="adresse" aria-describedby="adresseHelp">
                        <div id="adresseError" class="text-danger"></div>
                    </div>
                    <div class="col-sm-6 col-md-3">
                        <label class="form-label" for="numTel">NumTel</label>
                        <input type="text" class="form-control" id="numTel" name="numTel" aria-describedby="numTelHelp">
                        <div id="numTelError" class="text-danger"></div>
                    </div>
                    <div class="col-sm-6 col-md-3">
                        <label class="form-label" for="montant">Total Amount</label>
                        <input type="text" class="form-control" id="montant" name="montant" aria-describedby="montantHelp">
                    </div>
                    <!-- Submit button -->
...
<script>
    document.getElementById('form').addEventListener('submit', function(event) {
        ...
        // Validate adresse (more than 5 letters)
        if (adresseInput.value.length < 5) {
            adresseError.textContent = 'Adresse must be at least 5 letters.';
            adresseError.style.color = 'red';
            event.preventDefault();
        }
        
        // Validate numTel (8 numbers)
        if (!/^\d{8}$/.test(numTelInput.value)) {
            numTelError.textContent = 'NumTel must be 8 numbers.';
